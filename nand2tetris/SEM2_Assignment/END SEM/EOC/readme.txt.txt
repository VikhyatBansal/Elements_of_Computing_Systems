...
One must know how to work on JACK language to create .jack files (syntax, common errors, loops etc).
After creating .jack file one must use JACK COMPILER in nand2tetris folder to compile .jack file to .vm file so that one can check/test
jack file on VM Emulator in nand2tetris.
One can open the VM EMULATOR, load the compiled file and check whether the compiled file is serving the purpose or not.

Part 2-

One must know how to work on a programming language (python,java,C++ etc).
Here we are creating our own assembler from our knowledge on assembler and programming language PYTHON.
Using our own assembler we will create HACK file.
After creating the HACK file we are going to compare that HACK file using the IN-BUILT assembler in nand2tetris folder.

Part 3-

One must have all the created HDL files(chip files) so that when one loads the computer.hdl chip in hardware simulator, it loads into the simulator 
as all the chips are inter-related and must be present in the same folder.
After loading computer.HDL one can simply load asm/hack file in the hardware simulator and run that file in that hardware simulator.
